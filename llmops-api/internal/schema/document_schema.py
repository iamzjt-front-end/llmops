...
from wtforms.validators import AnyOf, DataRequired, Length, Optional, ValidationError
...
class CreateDocumentsReq(FlaskForm):
  ...
  process_type = StringField(
    ...
    validators=[
      ...
      AnyOf(
        values=[ProcessType.AUTOMATIC, ProcessType.CUSTOM], message='处理类型格式错误'
      ),
    ],
  )
  rule = DictField('rule')
  ...
  def validate_rule(self, field: DictField) -> None:
    ...
    if self.process_type.data == ProcessType.AUTOMATIC:
      ...
      for pre_process_rule in field.data['pre_process_rules']:
        # 5.校验id参数，非空、id规范
        if (
          not isinstance(pre_process_rule, dict)
          or 'id' not in pre_process_rule
          or pre_process_rule['id']
          not in ['remove_extra_space', 'remove_url_and_email']
        ):
