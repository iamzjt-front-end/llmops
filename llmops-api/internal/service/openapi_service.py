  ...
  def chat(self, req: OpenAPIChatReq, account: Account):
    ...
    if req.stream.data is True:
      ...
      def handle_stream() -> Generator:
        ...
        for agent_thought in agent.run(req.query.data, history, conversation.summary):
          # 提取thought以及answer
    ...
    # 17.块内容输出：当前Agent以统一事件流运行，这里聚合为普通JSON响应。
    agent_thoughts_dict = {}
    answer = ''
    latency = 0
    for agent_thought in agent.run(req.query.data, history, conversation.summary):
      if agent_thought.event != QueueEvent.PING:
        ...
        if agent_thought.event == QueueEvent.AGENT_MESSAGE:
          ...
          if event_id in agent_thoughts_dict:
            previous = agent_thoughts_dict[event_id]
            agent_thought = previous.model_copy(
              update={
                'thought': previous.thought + agent_thought.thought,
                'answer': previous.answer + agent_thought.answer,
                'latency': agent_thought.latency,
              }
            )
        agent_thoughts_dict[event_id] = agent_thought
