...
class ConversationService(BaseService):
  ...
  def generate_conversation_name(cls, query: str) -> str:
    ...
    try:
      ...
      logger.exception('提取会话名称出错, conversation_info: %s', conversation_info)
    if len(name) > 75:
      name = name[:75] + '...'
  ...
  def generate_suggested_questions(cls, histories: str) -> list[str]:
    ...
    try:
      ...
      logger.exception('生成建议问题出错, suggested_questions: %s', suggested_questions)
    if len(questions) > 3:
  ...
  def save_agent_thoughts(
    ...
    message_id: UUID,
    agent_thoughts: list[AgentQueueEvent],
  ):
    """存储智能体推理步骤消息"""
      ...
      for agent_thought in agent_thoughts:
        ...
        if agent_thought.event in [
          ...
          self.create(
            ...
            message=agent_thought.messages,
