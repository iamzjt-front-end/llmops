...
class AppService(BaseService):
  ...
  def debug_chat(self, app_id: UUID, query: str, account: Account) -> Generator:
    ...
    agent_thoughts = {}
    for agent_thought in agent.run(query, history, debug_conversation.summary):
  ...
  def stop_debug_chat(self, app_id: UUID, task_id: UUID, account: Account) -> None:
    ...
    task_owner = self.redis_client.get(belong_key)
    if isinstance(task_owner, bytes):
      task_owner = task_owner.decode()
    if task_owner != f'account-{account.id}':
      raise ForbiddenException('当前任务不存在或无权限停止')
    self.redis_client.setex(
      AgentQueueManager.generate_task_stopped_cache_key(task_id), 60, '1'
    )

  def get_debug_conversation_messages_with_page(
    self, app_id: UUID, req: GetDebugConversationMessagesWithPageReq, account: Account
    ...
    if req.created_at.data:
      # 4.将时间戳转换成DateTime
      created_at_datetime = datetime.fromtimestamp(req.created_at.data, UTC).replace(
        tzinfo=None
      )
    ...
    if 'datasets' in draft_app_config:
      ...
      for dataset_id in datasets:
        try:
          ...
          raise ValidateErrorException('知识库列表参数必须是UUID') from exc
      # 8.4 判断是否传递了重复的知识库
