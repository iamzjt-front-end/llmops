...
from internal.model import DatasetQuery, Document, KeywordTable, ProcessRule, Segment
...
class IndexingService(BaseService):
  ...
  def build_documents(self, document_ids: list[UUID]) -> None:
    ...
    for document in documents:
      try:
        ...
        self.update(
          document, status=DocumentStatus.PARSING, processing_started_at=datetime.now()
        )
  ...
  def update_document_enabled(self, document_id: UUID) -> None:
    ...
    try:
      ...
      # 5.记录日志并将状态修改回原来的状态
      logging.exception(
        f'修改向量数据库文档启用状态失败，文档id：{document_id}，错误信息：{str(e)}'
      )
